...
const T = {
  accent:"#00ff87",accentDim:"#00cc6a",accentGlow:"rgba(0,255,135,0.08)",accentDark:"#003d20",
  blue:"#60a5fa",yellow:"#fcd34d",red:"#f87171",purple:"#c084fc",orange:"#fb923c",teal:"#34d399",pink:"#f472b6",
  bg:"#050a07",bgMid:"#080f0a",card:"#0a120c",cardMid:"#0d1a0f",border:"#0f2214",borderMid:"#152b1a",
  textPrimary:"#f0fdf4",textMid:"#86efac",textDim:"#166534",voiceRate:0.9,voicePitch:0.97,
};
...
const ACTIVITIES = [
  {id:"walk",name:"Walking",icon:"🚶",calPerMin:4},{id:"run",name:"Running",icon:"🏃",calPerMin:10},
  {id:"yoga",name:"Yoga",icon:"🧘",calPerMin:3},{id:"cycle",name:"Cycling",icon:"🚴",calPerMin:7},
  {id:"swim",name:"Swimming",icon:"🏊",calPerMin:8},{id:"dance",name:"Dancing",icon:"💃",calPerMin:5},
  {id:"climb",name:"Stair Climb",icon:"🧗",calPerMin:9},{id:"stretch",name:"Stretching",icon:"🤸",calPerMin:2},
  {id:"hiit",name:"HIIT",icon:"⚡",calPerMin:12},{id:"cricket",name:"Cricket",icon:"🏏",calPerMin:5},
  {id:"football",name:"Football",icon:"⚽",calPerMin:8},{id:"badminton",name:"Badminton",icon:"🏸",calPerMin:6},
];
...
function loadData() {
  ...
  return {
    ...
    logged:[],water:[],activities:[],groceryChecked:[],streakDays:0,
    lastActiveDate:null,weeklyScores:[],cbtProgress:[],
    userProfile:{weight:70,height:170,age:25,gender:"other",name:""},
    aiScans:[],lastUpdated:Date.now(),
  };
}
function saveData(d){try{localStorage.setItem("manifix_nutrition_v92",JSON.stringify({...d,lastUpdated:Date.now()}));}catch{}}
...
function calcTotals(logged,water){
  ...
  todayLogs.forEach(log=>{
    if(log._barcode){
      totals.calories+=log._barcode.calories||0;totals.protein+=log._barcode.protein||0;
      totals.carbs+=log._barcode.carbs||0;totals.fat+=log._barcode.fat||0;totals.fiber+=log._barcode.fiber||0;
      totals[`${log._barcode.light||"yellow"}Count`]++;return;
    }
    if(log._ai){
      totals.calories+=log._ai.calories||0;totals.protein+=log._ai.protein||0;
      totals.carbs+=log._ai.carbs||0;totals.fat+=log._ai.fat||0;totals.fiber+=log._ai.fiber||0;
      totals[`${log._ai.light||"yellow"}Count`]++;return;
    }
    const f=FOOD_DB[log.foodId];if(!f)return;
    const p=log.portion||1;
    totals.calories+=f.calories*p;totals.protein+=f.protein*p;totals.carbs+=f.carbs*p;
    totals.fat+=f.fat*p;totals.fiber+=f.fiber*p;totals.iron+=(f.iron||0)*p;
    totals.calcium+=(f.calcium||0)*p;totals.vitC+=(f.vitC||0)*p;totals.vitD+=(f.vitD||0)*p;
...
function calcScore(totals,goal,burn){
  if(!goal)return 50;
  const net=totals.calories-burn;const calRatio=net/Math.max(goal.calories,1);
  const s=[
    ...
    Math.min(20,totals.water*2.5),Math.min(20,totals.greenCount*4),
  ];
  return Math.round(s.reduce((a,b)=>a+b,0));
}
function updateStreak(data){
  const today=todayStr();if(data.lastActiveDate===today)return data;
...
async function fetchBarcodeNutrition(barcode){
  ...
  const p=data.product;const n=p.nutriments||{};const kcal=n["energy-kcal_100g"]||0;
  return{
    name:p.product_name||p.product_name_en||"Unknown Product",brand:p.brands||"",
    image:p.image_url||null,quantity:p.quantity||"",calories:Math.round(kcal),
    protein:Math.round((n.proteins_100g||0)*10)/10,carbs:Math.round((n.carbohydrates_100g||0)*10)/10,
    fat:Math.round((n.fat_100g||0)*10)/10,fiber:Math.round((n.fiber_100g||0)*10)/10,
    sodium:Math.round((n.sodium_100g||0)*1000),sugar:Math.round((n["sugars_100g"]||0)*10)/10,
    light:kcal<100?"green":kcal<250?"yellow":"red",nutriscore:p.nutriscore_grade||"",
    serving:p.serving_size||"100g",countries:p.countries_tags?.[0]?.replace("en:","")||"",
  };
}

// ── AI IMAGE ANALYSIS (FIXED) ──
...
Respond ONLY with a valid JSON object. No markdown, no backticks, no extra text — pure JSON only:
{
  "dish_name": "Name of the overall dish or meal",
  "description": "Brief description of what you see (2 sentences max)",
  "confidence": "high",
  "serving_estimate": "estimated serving size",
  "total_calories": 450,
  "total_protein": 22,
  "total_carbs": 48,
  "total_fat": 15,
...
Rules:
- light must be "green" (under 400 cal), "yellow" (400-600 cal), or "red" (over 600 cal)
- health_score is 0-100 based on nutritional quality
- Be realistic for Indian food if applicable
- If not a food image, set dish_name to "Not a food image" and all numbers to 0`;
...
function makeSpeaker(){
  return function speak(text,urgent=false){
    ...
    const say=()=>{
      ...
      u.rate=urgent?1.0:T.voiceRate;u.pitch=T.voicePitch;
...
function injectCSS(){
  ...
  const el=document.createElement("style");el.id="nut-v92-css";
  el.textContent=`
...
function ScoreRing({score}){
  ...
  return(
    <svg width="100" height="100" viewBox="0 0 100 100">
      ...
      <text x="50" y="53" textAnchor="middle" fill={col} style={{fontSize:22,fontWeight:700,fontFamily:"'DM Mono',monospace"}}>{score}</text>
      <text x="50" y="66" textAnchor="middle" fill="#1e4d35" style={{fontSize:8,fontFamily:"'Syne',sans-serif",letterSpacing:".15em"}}>SCORE</text>
    </svg>
  );
}

function MacroBar({label,current,goal,color,unit="g"}){
  const pct=Math.min(100,Math.round((current/Math.max(goal,1))*100));const over=pct>=100;
  return(
    <div style={{marginBottom:10}}>
      ...
      <div style={{height:5,background:"#0a1a0a",borderRadius:3,overflow:"hidden"}}>
        <div style={{height:"100%",width:`${pct}%`,background:over?color:`${color}99`,transition:"width .7s",borderRadius:3,boxShadow:over?`0 0 8px ${color}88`:undefined}}/>
      </div>
    </div>
  );
}
  ...
  return(
    <div>
      ...
      <div style={{display:"flex",gap:5,marginBottom:10,flexWrap:"wrap"}}>
        {Array.from({length:goal},(_,i)=>(
          <div key={i} onClick={()=>i===count&&onAdd()}
            style={{width:28,height:34,borderRadius:4,overflow:"hidden",background:i<count?`${T.accent}15`:"#0a1a0a",border:`1.5px solid ${i<count?T.accent:"#0d2010"}`,cursor:i===count?"pointer":"default",transition:"all .25s",display:"flex",flexDirection:"column",justifyContent:"flex-end",boxShadow:i<count?`0 0 8px ${T.accent}33`:undefined}}>
      ...
      <button onClick={onAdd} disabled={count>=goal} className="btn ring"
        style={{width:"100%",padding:"10px",fontSize:12,fontWeight:700,background:count>=goal?`${T.accent}15`:T.accent,border:`2px solid ${count>=goal?T.accent:"#000"}`,color:count>=goal?T.accent:"#050a07",borderRadius:9,fontFamily:"'Syne',sans-serif",opacity:count>=goal?0.7:1,cursor:count>=goal?"not-allowed":"pointer",boxShadow:count>=goal?undefined:`0 0 16px ${T.accent}44`}}>
...
function SparkLine({scores}){
  if(!scores||scores.length<2)return <div style={{fontSize:11,color:T.textDim,textAlign:"center",padding:"16px 0"}}>Track more days to see trend</div>;
  const max=Math.max(...scores,1);const w=240,h=44;
  const pts=scores.map((s,i)=>`${(i/(scores.length-1))*w},${h-(s/max)*h}`).join(" ");
  return(
    <svg width={w} height={h} viewBox={`0 0 ${w} ${h}`} style={{overflow:"visible"}}>
      <defs><linearGradient id="sparkGrad" x1="0%" y1="0%" x2="100%" y2="0%"><stop offset="0%" stopColor={T.accent} stopOpacity="0.4"/><stop offset="100%" stopColor={T.accent} stopOpacity="1"/></linearGradient></defs>
      <polyline points={pts} fill="none" stroke="url(#sparkGrad)" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"/>
      {scores.map((s,i)=><circle key={i} cx={(i/(scores.length-1))*w} cy={h-(s/max)*h} r="4" fill={T.accent} stroke="#050a07" strokeWidth="2"/>)}
    </svg>
  );
}
...
function StreakBadge({days}){
  if(!days)return null;
  return <span style={{display:"inline-flex",alignItems:"center",gap:4,padding:"3px 10px",borderRadius:12,background:`${T.orange}15`,border:`1px solid ${T.orange}44`,fontSize:11,fontWeight:700,color:T.orange}}>🔥 {days}d streak</span>;
}

function Confetti({show}){
  if(!show)return null;
  const pieces=Array.from({length:24},(_,i)=>({id:i,left:`${Math.random()*100}%`,color:[T.accent,T.yellow,T.blue,T.orange,T.purple,T.pink][i%6],delay:`${Math.random()*.8}s`,dur:`${1+Math.random()*.6}s`}));
  return(
    <div style={{position:"fixed",inset:0,pointerEvents:"none",zIndex:9999,overflow:"hidden"}}>
      {pieces.map(p=><div key={p.id} style={{position:"absolute",left:p.left,top:"3%",width:9,height:9,borderRadius:p.id%3===0?50:2,background:p.color,animation:`confetti ${p.dur} ${p.delay} ease-in forwards`}}/>)}
    </div>
  );
}

// ── AI FOOD SCANNER (FIXED) ──
function AIFoodScanner({onFoodLogged,onClose}){
  ...
  const processFile=(file)=>{
    ...
    reader.onload=(e)=>{
      ...
      setImageData(full.split(",")[1]);setImageSrc(full);setMimeType(file.type);
      setPhase("preview");setError("");
    };
  ...
  const handleLog=()=>{
    if(!result)return;
    onFoodLogged({name:result.dish_name,calories:result.total_calories,protein:result.total_protein,carbs:result.total_carbs,fat:result.total_fat,fiber:result.total_fiber,light:result.light,meal,imageData:imageSrc});
    onClose();
  };
    ...
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.98)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:200,padding:16}}>
      <div style={{background:T.bg,border:`3px solid ${T.pink}`,padding:20,width:"min(500px,100%)",borderRadius:18,maxHeight:"94vh",overflowY:"auto"}} className="hide-scroll">
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:18}}>
          <div>
            <div style={{fontSize:18,fontWeight:800,color:T.textPrimary,fontFamily:"'Syne',sans-serif"}}>📸 AI Food Scanner</div>
            <div style={{fontSize:11,color:T.textDim,marginTop:2}}>Powered by Claude Vision · Instant calorie detection</div>
          </div>
        ...
        {phase==="upload"&&(
          <div className="fade-up">
            <div onDrop={handleDrop} onDragOver={e=>{e.preventDefault();setDragOver(true);}} onDragLeave={()=>setDragOver(false)} onClick={()=>fileRef.current?.click()}
              ...
              <div style={{fontSize:15,fontWeight:700,color:T.textPrimary,marginBottom:6,fontFamily:"'Syne',sans-serif"}}>Drop food photo here</div>
              <div style={{fontSize:12,color:T.textDim,marginBottom:16}}>or click to browse · JPG, PNG, WEBP</div>
              <button style={{padding:"10px 24px",fontSize:12,fontWeight:700,background:`${T.pink}20`,border:`2px solid ${T.pink}`,color:T.pink,borderRadius:8,cursor:"pointer",fontFamily:"'Syne',sans-serif"}}>Choose Photo</button>
            </div>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginTop:16}}>
              {[["🍛","Indian & World Food","Biryani, Dal, Pizza, Sushi"],["⚡","Instant Results","Under 5 seconds"],["🔬","Full Nutrition","Protein, carbs, fat, fiber"],["💡","Smart Tips","Health suggestions included"]].map(([icon,title,sub])=>(
        ...
        {phase==="preview"&&imageSrc&&(
          <div className="fade-up">
            ...
            <div style={{fontSize:12,color:T.textMid,textAlign:"center",marginBottom:14}}>Ready to analyze with Claude AI Vision</div>
            {loading?(
              ...
              <div style={{display:"flex",gap:10}}>
                <button onClick={()=>{setPhase("upload");setImageData(null);setImageSrc(null);}}
                  style={{flex:1,padding:11,fontSize:12,background:"#111",border:"1.5px solid #222",color:T.textDim,borderRadius:10,cursor:"pointer",fontFamily:"'Syne',sans-serif"}}>← Change Photo</button>
                <button onClick={analyze} className="btn ring"
                  style={{flex:2,padding:11,fontSize:13,fontWeight:700,background:T.pink,border:`2px solid ${T.pink}`,color:"#050a07",borderRadius:10,cursor:"pointer",fontFamily:"'Syne',sans-serif",boxShadow:`0 0 20px ${T.pink}44`}}>🔍 Analyze with AI</button>
              </div>
            )}
          </div>
        )}
        ...
        {phase==="result"&&result&&(
          <div className="fade-up">
            {imageSrc&&<img src={imageSrc} alt={result.dish_name} style={{width:"100%",maxHeight:180,objectFit:"cover",borderRadius:12,marginBottom:14,border:`2px solid ${lcfg.color}44`}}/>}
            <div style={{border:`2px solid ${lcfg.color}44`,background:lcfg.bg,padding:"14px 16px",borderRadius:12,marginBottom:12}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:10}}>
                <div style={{flex:1,marginRight:12}}>
                  ...
                  <div style={{fontSize:10,color:T.textDim,marginTop:4}}>Serving: {result.serving_estimate} · Confidence: {result.confidence}</div>
                </div>
                <div style={{textAlign:"center"}}>
                  ...
                  <div style={{fontSize:11,fontWeight:700,color:lcfg.color,marginTop:4}}>Health: {result.health_score}/100</div>
                </div>
              </div>
                ...
                <div style={{marginBottom:12}}>
                  ...
                  <div style={{display:"flex",flexWrap:"wrap",gap:5}}>
                    {result.items.map((item,i)=>(
                      <span key={i} style={{fontSize:10,padding:"3px 9px",borderRadius:12,background:`${T.pink}15`,border:`1px solid ${T.pink}33`,color:T.pink}}>{item.name} · {item.calories}cal ({item.quantity})</span>
                    ))}
                  </div>
                </div>
              )}
                ...
                <div style={{display:"flex",gap:5}}>
                  {["breakfast","lunch","dinner","snack"].map(m=>(
                    <button key={m} onClick={()=>setMeal(m)}
                      style={{flex:1,padding:"6px 4px",fontSize:10,fontWeight:600,textTransform:"capitalize",borderRadius:6,background:meal===m?`${T.accent}22`:"#0a1a0a",border:`1.5px solid ${meal===m?T.accent:T.border}`,color:meal===m?T.accent:T.textDim,cursor:"pointer",fontFamily:"'Syne',sans-serif"}}>{m}</button>
                  ))}
                </div>
              </div>
            </div>
            {result.tips&&result.tips.length>0&&(
              <div style={{border:`1px solid ${T.teal}33`,background:`${T.teal}06`,padding:"12px 14px",borderRadius:10,marginBottom:12}}>
                ...
                {result.tips.map((tip,i)=><div key={i} style={{fontSize:11,color:T.textMid,marginBottom:4,paddingLeft:10,borderLeft:`2px solid ${T.teal}44`}}>• {tip}</div>)}
              </div>
            )}
            {result.alternatives&&result.alternatives.length>0&&(
              <div style={{border:`1px solid ${T.yellow}33`,background:`${T.yellow}06`,padding:"12px 14px",borderRadius:10,marginBottom:12}}>
                ...
                {result.alternatives.map((alt,i)=><div key={i} style={{fontSize:11,color:T.textMid,marginBottom:4,paddingLeft:10,borderLeft:`2px solid ${T.yellow}44`}}>• {alt}</div>)}
              </div>
            )}
            <div style={{display:"flex",gap:10}}>
              <button onClick={()=>{setPhase("upload");setResult(null);setImageData(null);setImageSrc(null);}}
                style={{flex:1,padding:11,fontSize:12,background:"#111",border:"1.5px solid #222",color:T.textDim,borderRadius:10,cursor:"pointer",fontFamily:"'Syne',sans-serif"}}>📷 New Photo</button>
              <button onClick={handleLog} className="btn ring"
                style={{flex:2,padding:11,fontSize:13,fontWeight:700,background:T.accent,border:`2px solid ${T.accent}`,color:"#050a07",borderRadius:10,cursor:"pointer",fontFamily:"'Syne',sans-serif",boxShadow:`0 0 20px ${T.accent}44`}}>+ Log {result.total_calories} cal</button>
            </div>
          </div>
        )}
          ...
          <div className="fade-up" style={{textAlign:"center",padding:"24px 0"}}>
            ...
            <div style={{display:"flex",gap:10,justifyContent:"center"}}>
              <button onClick={()=>{setPhase("preview");setError("");}} className="btn ring"
                style={{padding:"10px 20px",fontSize:12,background:`${T.pink}15`,border:`2px solid ${T.pink}`,color:T.pink,borderRadius:10,cursor:"pointer",fontFamily:"'Syne',sans-serif"}}>Try Again</button>
...
function BarcodeScanner({onFoodLogged,onClose}){
  const [phase,setPhase]=useState("scan");const [barcode,setBarcode]=useState("");const [result,setResult]=useState(null);
  const [errMsg,setErrMsg]=useState("");const [loading,setLoading]=useState(false);const [portion,setPortion]=useState(100);const [meal,setMeal]=useState("snack");
  const inputRef=useRef();
  useEffect(()=>{if(phase==="scan")setTimeout(()=>inputRef.current?.focus(),100);},[phase]);
  const search=async(code)=>{
    const clean=code.trim();if(!clean)return;setLoading(true);setErrMsg("");
    try{const data=await fetchBarcodeNutrition(clean);setResult(data);setPortion(100);setPhase("result");}
    catch(e){setErrMsg(e.message||"Product not found.");setPhase("error");}
    finally{setLoading(false);}
  };
  ...
  const lcfg=result?(LIGHT_CONFIG[result.light]||LIGHT_CONFIG.yellow):LIGHT_CONFIG.yellow;
  const handleLog=()=>{onFoodLogged({name:result.name,brand:result.brand,calories:cal,protein,carbs,fat,fiber,light:result.light,portion,barcode,meal});onClose();};
  return(
        ...
        {phase==="result"&&result&&(
          <div className="fade-up">
            ...
            <div style={{display:"flex",gap:8}}>
              ...
              <button onClick={handleLog} className="btn ring"
                style={{flex:2,padding:11,fontSize:13,fontWeight:700,background:T.accent,border:`2px solid ${T.accent}`,color:"#050a07",borderRadius:10,cursor:"pointer",fontFamily:"'Syne',sans-serif"}}>+ Log {cal} cal</button>
            </div>
          </div>
        )}
      </div>
...
function CBTModule({cbtProgress,onComplete,onClose}){
  const completedIds=cbtProgress||[];const nextLesson=CBT_LESSONS.find(l=>!completedIds.includes(l.id))||CBT_LESSONS[0];
  const [activeLesson,setActiveLesson]=useState(nextLesson);const [phase,setPhase]=useState("read");const [exerciseInput,setExerciseInput]=useState("");
    ...
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.95)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:200,padding:20}}>
      <div style={{background:T.bg,border:`3px solid ${T.purple}`,padding:20,width:"min(440px,100%)",borderRadius:16,maxHeight:"92vh",overflowY:"auto"}} className="hide-scroll">
        ...
        <div className="fade-up" style={{border:`2px solid ${(catColor[activeLesson.category]||T.purple)}33`,background:`${(catColor[activeLesson.category]||T.purple)}08`,padding:"16px 18px",borderRadius:12,marginBottom:14}}>
          ...
          {phase==="exercise"&&(
            <>
              <div style={{fontSize:12,color:T.textMid,lineHeight:1.6,marginBottom:12,padding:"10px 12px",background:"rgba(0,0,0,.3)",borderRadius:8,borderLeft:`3px solid ${catColor[activeLesson.category]||T.purple}`}}>📝 {activeLesson.exercise}</div>
              ...
              <div style={{display:"flex",gap:8}}>
                ...
                <button onClick={()=>{onComplete(activeLesson.id);setPhase("done");}} className="btn ring"
                  style={{flex:2,padding:10,fontSize:12,fontWeight:700,background:T.purple,border:`2px solid ${T.purple}`,color:"#050a07",borderRadius:8,cursor:"pointer",fontFamily:"'Syne',sans-serif"}}>✓ Mark Complete</button>
              </div>
            </>
          )}
          {phase==="done"&&(
        ...
        <div style={{display:"grid",gap:6}}>
          {CBT_LESSONS.map(l=>{const done=completedIds.includes(l.id);const active=l.id===activeLesson.id;return(
...
function FoodLogModal({onClose,onLog}){
  const [search,setSearch]=useState("");const [sel,setSel]=useState(null);const [portion,setPortion]=useState(1);const [meal,setMeal]=useState("lunch");
  const filtered=useMemo(()=>{if(!search)return FOOD_KEYS.map(k=>({key:k,...FOOD_DB[k]}));const q=search.toLowerCase();return FOOD_KEYS.filter(k=>FOOD_DB[k].name.toLowerCase().includes(q)).map(k=>({key:k,...FOOD_DB[k]}));},[search]);
    ...
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.95)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:200,padding:16}}>
      <div style={{background:T.bg,border:`3px solid ${T.accent}`,padding:18,width:"min(480px,100%)",borderRadius:16,maxHeight:"92vh",overflowY:"auto"}} className="hide-scroll">
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
          <div><div style={{fontSize:16,fontWeight:700,color:T.textPrimary,fontFamily:"'Syne',sans-serif"}}>🍽️ Log Food</div><div style={{fontSize:11,color:T.textDim}}>Traffic-light system · {FOOD_KEYS.length} foods</div></div>
          <button onClick={onClose} style={{fontSize:20,background:"none",border:"none",color:"#444",cursor:"pointer"}}>✕</button>
        </div>
        ...
        <div style={{maxHeight:200,overflowY:"auto",marginBottom:12}} className="hide-scroll">
          {(search?[["search",filtered]]:Object.entries(lightGroups)).map(([light,foods])=>{
            if(!foods.length)return null;const lcfg=light!=="search"?LIGHT_CONFIG[light]:null;
            return(
              <div key={light} style={{marginBottom:12}}>
                ...
                <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:6}}>
                  {foods.slice(0,search?24:undefined).map(f=>{const fcfg=LIGHT_CONFIG[f.light]||LIGHT_CONFIG.yellow;return(
          ...
          <div className="fade-up" style={{border:`1.5px solid ${(LIGHT_CONFIG[sel.light]||LIGHT_CONFIG.yellow).color}44`,background:`${(LIGHT_CONFIG[sel.light]||LIGHT_CONFIG.yellow).color}08`,padding:"12px 14px",borderRadius:10,marginBottom:12}}>
            <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:10}}>
              ...
              <div style={{flex:1}}>
                ...
                <div style={{display:"flex",gap:6,marginTop:3}}><TrafficLight light={sel.light}/><span style={{fontSize:10,color:T.textDim}}>Satiety: {sel.satiety}/100</span></div>
              </div>
            </div>
...
function ActivityModal({onClose,onLog}){
  const [sel,setSel]=useState(null);const [mins,setMins]=useState(30);const calc=sel?Math.round(sel.calPerMin*mins):0;
  return(
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.95)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:200,padding:20}}>
      <div style={{background:T.bg,border:`3px solid ${T.orange}`,padding:20,width:"min(420px,100%)",borderRadius:16}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:7,marginBottom:14}}>
          {ACTIVITIES.map(a=>(
            <button key={a.id} onClick={()=>setSel(a)} className="btn ring"
              ...
              <div style={{fontSize:20,marginBottom:3}}>{a.icon}</div><div>{a.name}</div>
            </button>
          ))}
        </div>
        ...
        <div style={{display:"flex",gap:10}}>
          ...
          <button onClick={()=>{if(sel){onLog({activityId:sel.id,name:sel.name,icon:sel.icon,minutes:mins,calories:calc});onClose();}}} disabled={!sel} className="btn ring"
            style={{flex:1,padding:11,fontSize:13,fontWeight:700,background:sel?T.orange:"#1a1a1a",border:`2px solid ${sel?T.orange:"#333"}`,color:sel?"#050a07":T.textDim,borderRadius:10,cursor:sel?"pointer":"not-allowed",fontFamily:"'Syne',sans-serif"}}>Log Activity</button>
        </div>
      </div>
    </div>
  );
...
function ProfileModal({profile,dailyGoal,onSave,onClose}){
  const [p,setP]=useState({...profile});const [g,setG]=useState({...dailyGoal});
  const bmr=calcBMR(p);const tdee=Math.round(bmr*1.55);
  ...
  return(
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.95)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:200,padding:20}}>
      <div style={{background:T.bg,border:`3px solid ${T.blue}`,padding:20,width:"min(420px,100%)",borderRadius:16,maxHeight:"92vh",overflowY:"auto"}} className="hide-scroll">
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
          <div><div style={{fontSize:16,fontWeight:700,color:T.textPrimary,fontFamily:"'Syne',sans-serif"}}>⚙️ Profile & Goals</div><div style={{fontSize:11,color:T.textDim}}>Auto-calculates your BMR & TDEE</div></div>
          <button onClick={onClose} style={{fontSize:20,background:"none",border:"none",color:"#444",cursor:"pointer"}}>✕</button>
        </div>
        ...
        <div style={{marginBottom:6}}><div style={{fontSize:11,color:T.teal,textTransform:"uppercase",letterSpacing:".1em",marginBottom:8}}>Body Metrics</div>{field("Weight (kg)","weight",30,200)}{field("Height (cm)","height",100,220)}{field("Age","age",10,80)}</div>
          ...
          <div style={{display:"flex",gap:6}}>
            {["male","female","other"].map(g2=>(
              <button key={g2} onClick={()=>setP(prev=>({...prev,gender:g2}))}
                style={{flex:1,padding:"8px 4px",fontSize:11,fontWeight:600,textTransform:"capitalize",borderRadius:6,background:p.gender===g2?`${T.blue}22`:"#0a1a0a",border:`1.5px solid ${p.gender===g2?T.blue:T.border}`,color:p.gender===g2?T.blue:T.textDim,cursor:"pointer",fontFamily:"'Syne',sans-serif"}}>{g2}</button>
            ))}
          </div>
        </div>
        <div style={{marginTop:14,marginBottom:6}}><div style={{fontSize:11,color:T.orange,textTransform:"uppercase",letterSpacing:".1em",marginBottom:8}}>Daily Goals</div>{field("Calories","calories",1200,4000,50,true)}{field("Protein (g)","protein",30,200,5,true)}{field("Carbs (g)","carbs",100,400,10,true)}{field("Fat (g)","fat",20,150,5,true)}{field("Water (glasses)","water",4,15,1,true)}</div>
        <button onClick={()=>onSave(p,g)} className="btn ring"
          style={{width:"100%",padding:12,fontSize:13,fontWeight:700,background:T.accent,border:`2px solid ${T.accent}`,color:"#050a07",borderRadius:10,cursor:"pointer",fontFamily:"'Syne',sans-serif",boxShadow:`0 0 16px ${T.accent}44`,marginTop:10}}>✓ Save Profile</button>
      </div>
    </div>
  );
}
...
export default function NutritionHealthV9(){
  ...
  const [showFood,setShowFood]=useState(false);const [showActivity,setShowActivity]=useState(false);
  const [showBarcode,setShowBarcode]=useState(false);const [showAIScan,setShowAIScan]=useState(false);
  const [showCBT,setShowCBT]=useState(false);const [showProfile,setShowProfile]=useState(false);
  const [loading,setLoading]=useState(true);const [offline,setOffline]=useState(!navigator.onLine);
  const [confetti,setConfetti]=useState(false);const [activeTab,setActiveTab]=useState("today");const [celebMsg,setCelebMsg]=useState("");
  ...
  const mealPlan=useMemo(()=>{
    ...
    const meals={
      breakfast:[{name:"Oats + Banana + Milk",foods:["oats","banana","milk"],calories:290},{name:"Egg + Roti + Spinach",foods:["egg","roti","spinach"],calories:204},{name:"Idli + Sambar + Curd",foods:["idli","sambar","curd"],calories:292}],
      lunch:[{name:"Dal + Rice + Chicken + Carrot",foods:["dal","rice","chicken","carrot"],calories:452},{name:"Fish + Roti + Spinach + Tomato",foods:["fish","roti","spinach","tomato"],calories:351},{name:"Rajma + Rice + Salad",foods:["rajma","rice","salad"],calories:322}],
      dinner:[{name:"Dal + Roti + Curd",foods:["dal","roti","curd"],calories:279},{name:"Grilled Fish + Veggies",foods:["fish","spinach","tomato","carrot"],calories:288},{name:"Paneer + Roti + Salad",foods:["paneer","roti","salad"],calories:404}],
      snack:[{name:"Apple + Almonds",foods:["apple","almonds"],calories:216},{name:"Banana + Curd",foods:["banana","curd"],calories:148},{name:"Avocado + Broccoli",foods:["avocado","broccoli"],calories:215}],
    };
    return Object.fromEntries(Object.entries(meals).map(([k,v])=>[k,v[day%v.length]]));
  },[]);

  useEffect(()=>{
    injectCSS();const d=updateStreak(data);if(d.streakDays!==data.streakDays)setData(d);
    const t=setTimeout(()=>{setLoading(false);speak("Welcome to ManifiX Nutrition v9. AI Food Scanner ready.");},700);
  ...
  useEffect(()=>{
    if(score>=100){setConfetti(true);setCelebMsg("Perfect Score! 🎉");speak("Outstanding! Perfect nutrition score today!",true);setTimeout(()=>{setConfetti(false);setCelebMsg("");},3000);}
  },[score]);// eslint-disable-line

  const logFood=useCallback((entry)=>{setData(p=>({...p,logged:[...p.logged,{id:Date.now(),time:new Date().toISOString(),...entry}]}));speak("Food logged!");},[speak]);
  const logBarcodeFood=useCallback((scanResult)=>{setData(p=>({...p,logged:[...p.logged,{id:Date.now(),time:new Date().toISOString(),meal:scanResult.meal||"snack",_barcode:{name:scanResult.name,brand:scanResult.brand,calories:scanResult.calories,protein:scanResult.protein,carbs:scanResult.carbs,fat:scanResult.fat,fiber:scanResult.fiber,light:scanResult.light,portion:scanResult.portion,barcode:scanResult.barcode}}]}));speak(`Logged ${scanResult.calories} calories.`);},[speak]);
  const logAIFood=useCallback((aiResult)=>{setData(p=>({...p,logged:[...p.logged,{id:Date.now(),time:new Date().toISOString(),meal:aiResult.meal||"lunch",_ai:{name:aiResult.name,calories:aiResult.calories,protein:aiResult.protein,carbs:aiResult.carbs,fat:aiResult.fat,fiber:aiResult.fiber,light:aiResult.light,imageData:aiResult.imageData}}]}));speak(`AI detected ${aiResult.calories} calories!`);},[speak]);
  const addWater=useCallback(()=>{setData(p=>({...p,water:[...p.water,new Date().toISOString()]}));if(totals.water+1>=data.dailyGoal.water){speak("Hydration goal reached! Outstanding!",true);setConfetti(true);setTimeout(()=>setConfetti(false),2000);}},[totals.water,data.dailyGoal.water,speak]);
  const logActivity=useCallback((act)=>{setData(p=>({...p,activities:[...(p.activities||[]),{id:Date.now(),time:new Date().toISOString(),...act}]}));speak(`Burned ${act.calories} calories!`);},[speak]);
  const completeCBT=useCallback((id)=>{setData(p=>({...p,cbtProgress:[...(p.cbtProgress||[]),id]}));speak("Psychology lesson complete!");},[speak]);
  const toggleGrocery=useCallback((item)=>{setData(p=>({...p,groceryChecked:p.groceryChecked.includes(item)?p.groceryChecked.filter(i=>i!==item):[...p.groceryChecked,item]}));},[]);
  const saveProfile=useCallback((profile,dailyGoal)=>{setData(p=>({...p,userProfile:profile,dailyGoal}));setShowProfile(false);speak("Profile updated!");},[speak]);
  ...
  return(
    <div style={{minHeight:"100dvh",background:T.bg,color:T.textPrimary,fontFamily:"'Syne',sans-serif",display:"flex",flexDirection:"column",alignItems:"center",position:"relative"}}>
      ...
      {offline&&<div style={{position:"fixed",top:10,left:"50%",transform:"translateX(-50%)",zIndex:99,fontSize:10,letterSpacing:".1em",background:T.card,border:`2px solid ${T.yellow}`,color:T.yellow,padding:"5px 14px",textTransform:"uppercase",borderRadius:6}}>⚠️ Offline — AI Scanner & Barcode need internet</div>}
      {celebMsg&&<div style={{position:"fixed",top:60,left:"50%",transform:"translateX(-50%)",zIndex:150,fontSize:14,fontWeight:800,background:T.accent,color:"#050a07",padding:"10px 22px",borderRadius:10,fontFamily:"'Syne',sans-serif",boxShadow:`0 0 24px ${T.accent}66`}}>{celebMsg}</div>}
      ...
      <div style={{position:"relative",zIndex:2,width:"min(500px,98vw)",display:"flex",flexDirection:"column",gap:12,padding:"18px 0 70px"}}>
        ...
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",paddingBottom:14,borderBottom:`1.5px solid ${T.border}`}}>
          <div>
            <div style={{fontFamily:"'Syne',sans-serif",fontSize:24,fontWeight:800,lineHeight:1.1,color:T.textPrimary}}>ManifiX <span style={{color:T.accent,textShadow:`0 0 20px ${T.accent}66`}}>Nutrition</span></div>
            ...
            <div style={{marginTop:8,display:"flex",gap:6,flexWrap:"wrap"}}>
              ...
              {cbtDone>0&&<span style={{display:"inline-flex",alignItems:"center",gap:4,padding:"3px 10px",borderRadius:12,background:`${T.purple}15`,border:`1px solid ${T.purple}44`,fontSize:11,fontWeight:700,color:T.purple}}>🧠 {cbtDone} lessons</span>}
              <span style={{display:"inline-flex",alignItems:"center",gap:4,padding:"3px 10px",borderRadius:12,background:`${T.blue}12`,border:`1px solid ${T.blue}33`,fontSize:11,color:T.blue}}>BMR: {bmr}</span>
            </div>
          </div>
          <div style={{display:"flex",flexDirection:"column",gap:6,alignItems:"flex-end"}}>
            {burnCal>0&&<div style={{fontSize:11,color:T.orange,background:`${T.orange}12`,border:`1px solid ${T.orange}33`,padding:"4px 10px",borderRadius:8,fontFamily:"'DM Mono',monospace"}}>🔥 -{burnCal} burned</div>}
            <button onClick={()=>setShowProfile(true)} className="btn ring"
              style={{fontSize:10,background:`${T.accent}12`,border:`1px solid ${T.accent}33`,color:T.accent,padding:"4px 10px",borderRadius:8,cursor:"pointer",fontFamily:"'Syne',sans-serif"}}>⚙️ Profile</button>
          </div>
        </div>
        ...
        <div className="fade-up" style={{display:"flex",gap:12}}>
          <div style={{border:`2px solid ${scoreColor}33`,background:`${scoreColor}07`,padding:"14px 12px",borderRadius:14,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",minWidth:118}}>
            ...
            <div style={{fontSize:10,fontWeight:700,color:scoreColor,marginTop:5}}>{score>=80?"Excellent":score>=60?"Good":"Keep going"}</div>
        ...
        <div className="fade-up" style={{border:`2px solid ${T.pink}44`,background:`${T.pink}07`,padding:"14px 16px",borderRadius:14,position:"relative",overflow:"hidden"}}>
          ...
          <div style={{display:"flex",alignItems:"center",gap:12}}>
            ...
            <div style={{flex:1}}>
              ...
              <div style={{fontSize:11,color:T.textDim,marginTop:2}}>Photo → ManifiX AI</div>
            ...
            <button onClick={()=>setShowAIScan(true)} className="btn ring"
              style={{padding:"10px 16px",fontSize:12,fontWeight:800,background:T.pink,border:"none",color:"#050a07",borderRadius:10,cursor:"pointer",fontFamily:"'Syne',sans-serif",boxShadow:`0 0 20px ${T.pink}55`,whiteSpace:"nowrap"}}>📸 Scan Food</button>
          </div>
        </div>

        {/* TRAFFIC LIGHTS */}
        <div className="fade-up" style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:8}}>
          {Object.entries(LIGHT_CONFIG).map(([key,cfg])=>{const count=totals[`${key}Count`]||0;return(
        ...
        <div>
          ...
          <div style={{display:"flex",gap:6,overflowX:"auto",paddingBottom:4}} className="hide-scroll">
            {QUICK_ADD.map(key=>{const f=FOOD_DB[key];const lcfg=LIGHT_CONFIG[f.light]||LIGHT_CONFIG.yellow;return(
        ...
        <div style={{display:"grid",gridTemplateColumns:"2fr 1fr 1fr",gap:8}}>
          <button onClick={()=>setShowFood(true)} className="btn ring"
            style={{padding:"13px 8px",fontSize:13,fontWeight:800,background:T.accent,border:"none",color:"#050a07",borderRadius:10,fontFamily:"'Syne',sans-serif",display:"flex",alignItems:"center",justifyContent:"center",gap:5,cursor:"pointer",boxShadow:`0 0 20px ${T.accent}44`}}>🍽️ Log Food</button>
          <button onClick={()=>setShowBarcode(true)} className="btn ring"
            style={{padding:"13px 6px",fontSize:11,fontWeight:700,background:`${T.teal}18`,border:`2px solid ${T.teal}`,color:T.teal,borderRadius:10,fontFamily:"'Syne',sans-serif",display:"flex",alignItems:"center",justifyContent:"center",gap:4,cursor:"pointer"}}>🏷️ Barcode</button>
          <button onClick={()=>setShowActivity(true)} className="btn ring"
            style={{padding:"13px 6px",fontSize:11,fontWeight:700,background:`${T.orange}15`,border:`2px solid ${T.orange}`,color:T.orange,borderRadius:10,fontFamily:"'Syne',sans-serif",display:"flex",alignItems:"center",justifyContent:"center",gap:4,cursor:"pointer"}}>🏃 Burns</button>
        </div>
        ...
        {activeTab==="today"&&(
          <div className="fade-up">
            <div style={{fontSize:11,color:T.textDim,textTransform:"uppercase",letterSpacing:".1em",marginBottom:8}}>Today's Log · {todayLog.length} entries</div>
            {todayLog.length===0?(
              <div style={{textAlign:"center",padding:"28px 0",color:T.textDim,fontSize:12,border:`1.5px dashed ${T.border}`,borderRadius:12}}>No food logged yet today.<br/><span style={{fontSize:11,opacity:.6}}>Use AI Scanner, Barcode or Log Food above</span></div>
            ):(
              <div style={{display:"grid",gap:6}}>
                {todayLog.map(log=>{
                  if(log._ai){const a=log._ai;const acfg=LIGHT_CONFIG[a.light]||LIGHT_CONFIG.yellow;return(
                    <div key={log.id} style={{display:"flex",alignItems:"flex-start",gap:10,padding:"10px 12px",background:T.card,border:`1.5px solid ${T.pink}44`,borderRadius:10}}>
                      {a.imageData?<img src={a.imageData} alt={a.name} style={{width:44,height:44,borderRadius:8,objectFit:"cover",flexShrink:0}}/>:<span style={{fontSize:22}}>📸</span>}
                      <div style={{flex:1}}>
                        ...
                        <div style={{display:"flex",gap:8,fontSize:10,color:T.textDim,marginTop:3}}>
                          <span style={{color:T.blue}}>P:{a.protein}g</span><span style={{color:T.yellow}}>C:{a.carbs}g</span><span style={{color:T.red}}>F:{a.fat}g</span><TrafficLight light={a.light}/>
                        </div>
                      </div>
                    </div>
                  );}
                  if(log._barcode){const b=log._barcode;const bcfg=LIGHT_CONFIG[b.light]||LIGHT_CONFIG.yellow;return(
                    <div key={log.id} style={{display:"flex",alignItems:"flex-start",gap:10,padding:"10px 12px",background:T.card,border:`1.5px solid ${T.teal}44`,borderRadius:10}}>
                      ...
                      <div style={{flex:1}}>
                        ...
                        <div style={{display:"flex",gap:8,fontSize:10,color:T.textDim,marginTop:3}}>
                          <span style={{color:T.blue}}>P:{b.protein}g</span><span style={{color:T.yellow}}>C:{b.carbs}g</span><span style={{color:T.red}}>F:{b.fat}g</span><TrafficLight light={b.light}/>
                        </div>
                      </div>
                    </div>
                  );}
                  const f=FOOD_DB[log.foodId];if(!f)return null;const lcfg=LIGHT_CONFIG[f.light]||LIGHT_CONFIG.yellow;
                  return(
            ...
            {Object.entries(mealPlan).map(([type,plan])=>(
              <div key={type} style={{border:`1.5px solid ${T.border}`,background:T.card,padding:"12px 14px",borderRadius:12,marginBottom:8}}>
                ...
                <div style={{display:"flex",gap:5,flexWrap:"wrap",marginBottom:8}}>
                  {plan.foods?.map((fid,i)=>{const f=FOOD_DB[fid];if(!f)return null;const lcfg=LIGHT_CONFIG[f.light]||LIGHT_CONFIG.yellow;return <span key={i} style={{fontSize:9,padding:"2px 7px",borderRadius:4,background:lcfg.bg,color:lcfg.color}}>{f.icon} {f.name.split(" ")[0]}</span>;})}
                </div>
                <button onClick={()=>plan.foods?.forEach(fid=>logFood({foodId:fid,meal:type,portion:1}))} className="btn ring"
                  style={{width:"100%",padding:"8px",fontSize:11,fontWeight:700,background:`${T.accent}10`,border:`1px solid ${T.accent}`,color:T.accent,borderRadius:6,cursor:"pointer",fontFamily:"'Syne',sans-serif"}}>✓ Log This Meal</button>
              </div>
            ))}
          </div>
        )}
        ...
        {activeTab==="grocery"&&(
          <div className="fade-up">
            ...
            <div style={{display:"grid",gap:5}}>
              {grocery.map(item=>{const checked=data.groceryChecked.includes(item);return(
        ...
        {activeTab==="trends"&&(
          <div className="fade-up">
            <div style={{border:`1.5px solid ${T.border}`,background:T.card,padding:"14px 16px",borderRadius:12,marginBottom:10}}>
              ...
              <div style={{display:"flex",justifyContent:"center",marginBottom:8}}><SparkLine scores={[...(data.weeklyScores||[]).slice(-6),score]}/></div>
              <div style={{display:"flex",justifyContent:"space-between",fontSize:10,color:T.textDim}}>
                <span>7 days ago</span><span style={{color:scoreColor,fontFamily:"'DM Mono',monospace"}}>Today: {score}</span>
              </div>
            </div>
                ...
                <div style={{display:"grid",gap:5}}>
                  {todayLog.filter(l=>l._ai).map(log=>(
                    <div key={log.id} style={{display:"flex",gap:10,alignItems:"center"}}>
                      ...
                      <div style={{flex:1}}><div style={{fontSize:11,color:T.textPrimary}}>{log._ai.name}</div><div style={{fontSize:10,color:T.textDim}}>{log._ai.calories} cal · {log.meal}</div></div>
                    </div>
                  ))}
                </div>
              </div>
                ...
                <div style={{display:"grid",gap:6}}>
                  {(data.activities||[]).filter(a=>a.time.startsWith(todayStr())).map(a=>(
                    <div key={a.id} style={{display:"flex",alignItems:"center",gap:10,padding:"8px 12px",background:"#0a1a0a",border:`1px solid ${T.border}`,borderRadius:8}}>
                      ...
                      <div style={{flex:1}}><div style={{fontSize:12,fontWeight:600,color:T.textPrimary}}>{a.name}</div><div style={{fontSize:10,color:T.textDim}}>{a.minutes} min</div></div>
            ...
            <div style={{border:`1.5px solid ${T.purple}22`,background:`${T.purple}06`,padding:"12px 14px",borderRadius:12}}>
              ...
              <div style={{display:"flex",gap:4,flexWrap:"wrap",marginBottom:8}}>
                {CBT_LESSONS.map(l=>{const done=(data.cbtProgress||[]).includes(l.id);return(
                  <div key={l.id} style={{width:34,height:34,borderRadius:7,background:done?`${T.purple}22`:"#0a1a0a",border:`1.5px solid ${done?T.purple:T.border}`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:15}}>{done?"✓":l.icon}</div>
                );})}
              </div>
        ...
        <div style={{textAlign:"center",fontSize:9,letterSpacing:".12em",color:T.textDim,textTransform:"uppercase",paddingTop:8}}>
         ManifiX  AI · {offline?"Offline":"Online ✓"}
        </div>
      </div>
