...
const stressPrograms = [
  {
    ...
    level: "Beginner",
    icon: TimerReset,
    description: "Quick stress relief for busy moments. Focuses on instant nervous system reset through breath pacing.",
    steps: ["Find a quiet spot", "Inhale 4 seconds", "Hold 4 seconds", "Exhale 6 seconds", "Repeat 8 cycles"],
    category: "breathing",
  },
  {
    id: 2,
    ...
    level: "Popular",
    icon: Brain,
    description: "Targeted anxiety reduction using cognitive reframing and grounding techniques.",
    steps: ["Acknowledge the anxiety", "Name 5 things you see", "Name 4 things you touch", "Name 3 sounds", "Challenge one thought", "Breathe slowly", "Accept what you cannot control"],
    category: "anxiety",
  },
  {
    id: 3,
    ...
    level: "Daily",
    icon: HeartPulse,
    description: "Diaphragmatic breathing to activate the parasympathetic nervous system.",
    steps: ["Place hand on belly", "Breathe in through nose", "Feel belly expand", "Exhale through pursed lips", "Feel belly contract", "Find your rhythm"],
    category: "breathing",
  },
  {
    id: 4,
    ...
    level: "Night",
    icon: Moon,
    description: "Progressive muscle relaxation and body scan for deep sleep preparation.",
    steps: ["Lie comfortably", "Close your eyes", "Scan from toes upward", "Tense each muscle group", "Hold for 5 seconds", "Release tension", "Notice the warmth", "Let go of thoughts"],
    category: "sleep",
  },
  {
    id: 5,
    ...
    level: "Beginner",
    icon: Wind,
    description: "Combine movement with mindfulness to reduce cortisol and improve mood.",
    steps: ["Start walking slowly", "Feel each footstep", "Notice your surroundings", "Match breath to steps", "Release mental tension", "End with gratitude"],
    category: "movement",
  },
  {
    id: 6,
    title: "Workplace Decompression",
    duration: 3,
    level: "Daily",
    icon: Briefcase,
    description: "Quick desk-friendly exercises to reduce work stress without leaving your station.",
    steps: ["Roll shoulders back 5 times", "Neck stretches side to side", "Close eyes, deep breath", "Unclench jaw", "Stretch arms overhead", "Reset posture"],
    category: "workplace",
  },
];
...
export default function StressHealth() {
  ...
  const [moodHistory, setMoodHistory] = useState(() => {
    const saved = localStorage.getItem("stressMoods");
  ...
  const [journalEntries, setJournalEntries] = useState(() => {
    const saved = localStorage.getItem("stressJournal");
    return saved ? JSON.parse(saved) : [];
  });
  ...
  const [activeTriggers, setActiveTriggers] = useState(() => {
    const saved = localStorage.getItem("stressTriggers");
    return saved ? JSON.parse(saved) : [];
  });
  const [goals, setGoals] = useState(() => {
    const saved = localStorage.getItem("stressGoals");
    return saved ? JSON.parse(saved) : [
  ...
  useEffect(() => {
    localStorage.setItem("stressMoods", JSON.stringify(moodHistory));
  }, [moodHistory]);

  useEffect(() => {
    localStorage.setItem("stressJournal", JSON.stringify(journalEntries));
  }, [journalEntries]);

  useEffect(() => {
    localStorage.setItem("stressTriggers", JSON.stringify(activeTriggers));
  }, [activeTriggers]);

  useEffect(() => {
    localStorage.setItem("stressGoals", JSON.stringify(goals));
  }, [goals]);
    ...
    programInterval.current = setInterval(() => {
      setProgramTime((prev) => {
        ...
        if (currentStep >= program.steps.length) {
          ...
          setGoals((prev) =>
            prev.map((g) =>
              g.id === 3 ? { ...g, done: true } : g
            )
          );
          return newTime;
        }
        setProgramStep(currentStep);
        const phase = Math.floor((newTime % stepDuration) / (stepDuration / 4));
  ...
  const selectMood = useCallback(
    (mood) => {
      ...
      if (mood === "Peaceful" || mood === "Calm" || mood === "Energized") {
        setStreak((prev) => prev + 1);
      }
      setGoals((prev) =>
        prev.map((g) =>
          g.id === 2 ? { ...g, done: true } : g
        )
      );
    },
    [setMoodHistory, setStreak, setGoals]
  );
  ...
  const getStressLevel = (score) => {
    ...
    if (score <= 60) return { label: "Moderate", color: "#D4AF37" };
  ...
  const renderHome = () => (
    <div className="space-y-8">
      <motion.div
        ...
        style={{
          background: "linear-gradient(135deg, rgba(212,175,55,0.1) 0%, rgba(10,10,10,0.95) 50%, rgba(212,175,55,0.05) 100%)",
          border: "1px solid rgba(212,175,55,0.15)",
        }}
      >
        <div className="absolute -top-20 -right-20 w-80 h-80 rounded-full opacity-20" style={{ background: "radial-gradient(circle, rgba(212,175,55,0.4), transparent)", filter: "blur(60px)" }} />
        <div className="relative z-10">
          ...
          <p className="text-gray-400 text-lg mt-5 max-w-2xl leading-relaxed">
            Guided breathing, emotional resets, AI-powered calm routines, sleep recovery, and burnout prevention — built for modern life.
          </p>
          <div className="flex flex-wrap gap-4 mt-8">
            <button onClick={() => setActiveTab("programs")} className="px-6 py-4 rounded-2xl bg-gradient-to-r from-yellow-500 to-amber-500 text-black font-bold hover:scale-105 transition-all shadow-lg shadow-yellow-500/30">
              Start Recovery Session
            </button>
            <button onClick={() => setShowQuiz(true)} className="px-6 py-4 rounded-2xl border border-yellow-500/30 bg-yellow-500/10 text-yellow-300 hover:bg-yellow-500/20 transition-all">
    ...
    <div className="space-y-6">
      ...
      <div className="grid sm:grid-cols-2 gap-4">
        {stressPrograms.map((program) => {
          ...
          return (
            <motion.div key={program.id} whileHover={{ y: -4 }} className="rounded-2xl p-5 cursor-pointer transition-all" style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(212,175,55,0.1)" }} onClick={() => startProgram(program)}>
        ...
        {activeProgram && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-40 bg-black/90 backdrop-blur-md flex items-center justify-center p-4">
            <div className="text-center max-w-md w-full">
              ...
              <motion.div
                animate={
                  breathingPhase === "inhale" ? { scale: 1.4 } : breathingPhase === "exhale" ? { scale: 0.9 } : breathingPhase === "hold" ? { scale: 1.4 } : { scale: 1 }
                }
                ...
                style={{ background: breathingPhase === "done" ? "rgba(52,211,153,0.2)" : "radial-gradient(circle, rgba(212,175,55,0.3), rgba(212,175,55,0.05))", border: "2px solid rgba(212,175,55,0.3)" }}
              >
              ...
              <div className="flex items-center justify-center gap-4 mb-6">
                ...
                <button onClick={stopProgram} className="w-12 h-12 rounded-xl bg-red-500/20 flex items-center justify-center text-red-400">
                  <X size={24} />
                </button>
              </div>
    ...
    <div className="space-y-6">
      ...
      <GoldCard className="p-6">
        ...
        <div className="grid grid-cols-3 md:grid-cols-5 gap-3">
          {moodEmojis && Object.entries(moodEmojis).map(([mood, emoji]) => (
          ...
          <div className="h-56">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={moodChartData}>
                ...
                <Tooltip contentStyle={{ background: "rgba(10,10,10,0.95)", border: "1px solid rgba(212,175,55,0.2)", borderRadius: "12px", color: "#fff" }} />
                <Area type="monotone" dataKey="value" stroke="#D4AF37" fill="url(#moodGrad)" />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        ) : (
        ...
        <GoldCard className="p-6">
          ...
          {selectedCoping && (
            <div className="mt-4 p-3 rounded-xl bg-white/5 border border-white/5">
              ...
              <button onClick={() => { setCopingActive(true); setCopingTime(0); const minutes = parseInt(selectedCoping.time); copingInterval.current = setInterval(() => setCopingTime((p) => p + 1), 1000); setTimeout(() => { if (copingInterval.current) clearInterval(copingInterval.current); setCopingActive(false); }, minutes * 60 * 1000); }} className="w-full mt-3 py-2 rounded-lg bg-yellow-500/20 text-yellow-300 text-sm hover:bg-yellow-500/30 transition-colors">{copingActive ? "In progress..." : "Start Now"}</button>
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-white relative overflow-hidden">
      ...
      <div className="fixed top-0 right-0 w-[500px] h-[500px] opacity-15" style={{ background: "radial-gradient(circle, rgba(212,175,55,0.15), transparent)", filter: "blur(100px)" }} />

      <header className="relative z-20 border-b border-white/5 bg-black/50 backdrop-blur-xl sticky top-0">
        <div className="max-w-7xl mx-auto px-5 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl flex items-center justify-center" style={{ background: "linear-gradient(135deg, #D4AF37, #FFD700, #B8960C)" }}>
              ...
              <GoldCard className="p-6">
                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-between p-4 rounded-xl bg-white/5">
                    ...
                    <button onClick={() => setSoundEnabled(!soundEnabled)} className="w-10 h-6 rounded-full bg-yellow-500/30 flex items-center px-0.5"><div className="w-5 h-5 rounded-full bg-yellow-400" /></button>
                  </div>
                  <button onClick={exportData} className="w-full flex items-center gap-3 p-4 rounded-xl bg-white/5 hover:bg-yellow-500/10 transition-colors"><Download size={18} className="text-yellow-400" /><div className="text-left"><p className="text-sm font-medium text-white">Export Data</p><p className="text-xs text-gray-500">Download your health data</p></div></button>
                  <button onClick={resetData} className="w-full flex items-center gap-3 p-4 rounded-xl bg-white/5 hover:bg-red-500/10 transition-colors"><RefreshCw size={18} className="text-red-400" /><div className="text-left"><p className="text-sm font-medium text-white">Reset All Data</p><p className="text-xs text-gray-500">Clear everything</p></div></button>
                </div>
              </GoldCard>
            </motion.div>
          </div>
  ...
  return (
    <div className="text-center">
      <motion.div
        ...
        style={{ background: phase === "idle" ? "rgba(255,255,255,0.05)" : "radial-gradient(circle, rgba(212,175,55,0.3), rgba(212,175,55,0.05))", border: "2px solid rgba(212,175,55,0.3)" }}
      >
      ...
      <button onClick={phase === "idle" ? start : stop} className={`px-6 py-2 rounded-xl text-sm font-semibold transition-all ${phase === "idle" ? "bg-yellow-500/20 text-yellow-300 hover:bg-yellow-500/30" : "bg-red-500/20 text-red-300 hover:bg-red-500/30"}`}>{phase === "idle" ? "Start Exercise" : "Stop"}</button>
    </div>
  );
}
